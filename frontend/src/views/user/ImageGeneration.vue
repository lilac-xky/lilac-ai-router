        ...
        <a-row :gutter="16">
                ...
                <a-card title="AI 文生图" :loading="generating">
                    <a-form layout="vertical">
                        <a-form-item label="描述你想要的图片">
                            <a-textarea v-model:value="formData.prompt" placeholder="例如：一只可爱的猫咪在花园里玩耍，阳光明媚，卡通风格"
                        ...
                        <a-alert v-if="estimatedCost > 0" :message="`预估费用：¥${estimatedCost.toFixed(4)}`" type="info"
                            show-icon style="margin-bottom: 16px" />

                        <a-form-item>
                            <a-button type="primary" size="large" :loading="generating" @click="handleGenerate" block>
                                {{ generating ? '生成中...' : '开始生成' }}
                            </a-button>
                        </a-form-item>
                    </a-form>
                    ...
                    <div v-if="generatedImages.length > 0" style="margin-top: 24px">
                        ...
                        <a-row :gutter="16">
                            <a-col v-for="(image, index) in generatedImages" :key="index" :span="24">
                                <a-card hoverable>
            ...
            <a-col :span="12">
                <a-card title="生成历史" :loading="loading">
                    <a-list :data-source="historyRecords" :pagination="pagination" @change="handlePageChange">
                            ...
                            <a-list-item>
                                <a-list-item-meta>
                                    ...
                                    <template #avatar v-if="item.imageUrl && item.status === 'success'">
                                        <a-image :src="item.imageUrl" :width="80" :height="80" :preview="true"
