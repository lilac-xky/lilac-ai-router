    ...
    <div id="blacklistManagePage">
        ...
        <a-card title="添加 IP 到黑名单" :bordered="false" style="margin-bottom: 16px">
            <a-form layout="inline" :model="addForm" @finish="doAdd">
                <a-form-item label="IP 地址">
                    <a-input v-model:value="addForm.ip" placeholder="输入 IP 地址，如 192.168.1.1" style="width: 240px"
                        allow-clear />
                </a-form-item>
                <a-form-item label="封禁原因">
                    <a-input v-model:value="addForm.reason" placeholder="可选，填写封禁原因" style="width: 240px" allow-clear />
                </a-form-item>
                <a-form-item>
        ...
        <a-card title="黑名单列表" :bordered="false">
            ...
            <a-spin :spinning="loading">
                ...
                <a-list v-else :data-source="blacklist"
                    :grid="{ gutter: 16, xs: 1, sm: 2, md: 3, lg: 4, xl: 4, xxl: 6 }">
                    <template #renderItem="{ item }">
                        <a-list-item>
                            <a-card hoverable size="small">
                                <template #actions>
                                    <a-popconfirm title="确定要移除该 IP 吗？" ok-text="确定" cancel-text="取消"
...
// 添加表单
const addForm = reactive<API.BlacklistRequest>({
    ip: '',
    reason: '',
})
...
const doAdd = async () => {
    ...
    if (blacklist.value.includes(ip)) {
        message.warning('该 IP 已在黑名单中')
        return
    }

    addLoading.value = true
    try {
        const res = await addToBlacklist({ ip, reason: addForm.reason?.trim() || undefined })
        if (res.data.code === 200) {
...
const doRemove = async (ip: string) => {
    try {
        ...
        if (res.data.code === 200) {
            ...
            message.error(res.data.msg || '移除失败')
        }
    } catch (e) {
        message.error('移除失败')
    }
